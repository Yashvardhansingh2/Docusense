...
## Deployment (Render — fully free)

Every service used by Docusense has a **free tier**. No credit card is required.

| Service | What it does | Free tier |
|---|---|---|
| [Render](https://render.com) | Hosts the API | Free web service (spins down after 15 min inactivity) |
| [Supabase](https://supabase.com) | PostgreSQL + pgvector | 500 MB DB, 1 GB file storage |
| [Upstash Redis](https://upstash.com) | Query cache | 10 000 commands / day |
| [Google AI Studio](https://aistudio.google.com) | Gemini LLM + embeddings | Free tier (15 RPM) |

A [`render.yaml`](render.yaml) blueprint is included for one-click deployment.

### Steps

1. **Create free accounts** for Supabase, Upstash, and Google AI Studio (links above).
2. **Run [`supabase_setup.sql`](supabase_setup.sql)** in your Supabase SQL editor to create tables and the pgvector index.
3. **Push this repo to GitHub** (if not already).
4. **Log in to [Render](https://render.com)** and click **New → Blueprint**.
5. **Connect your GitHub repository** — Render will detect `render.yaml` automatically and select the **free** plan.
6. **Set the secret environment variables** in the Render dashboard.
...
7. **Apply the blueprint** — Render builds the Docker image and starts the service. `SECRET_KEY` is auto-generated.

> **Free-tier note:** Render's free web service spins down after 15 minutes of inactivity. The first request after a cold start may take ~30 seconds. All subsequent requests are fast.
